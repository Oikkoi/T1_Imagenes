...
1. *formar_intervalos*: Recibe como argumento una lista de tuplas, en formato `(h_i, m_i)` y returna los arrays de los *hue* y los pesos *m* ordenados. (Es decir, la posición i en ambos array corresponde al par ordenado `h_i`, `m_i`). Para hacerlo, utiliza la función `numpy.array()` para convertir las listas unzipped en arrays. Retorna dichos arrays.
